...
| ---------------- | -------------------- | ----------------- | ----------------- | ----------------------------- | ------------------------ | ------------------------------------------------------------ | ---------------------------------- | ----------------------- |
| **①**            | 策略分流 DNS         | **全 No-resolve** | ON                | ⚠️ Proxy                       | ✅ Proxy                  | 🟢 **高**：国外/其他域名走机场 DoH，不因 IP 规则额外 Resolve  | 隐私强，但未知国内可能误走代理     | 🟡 Mac 可用              |
| **②**            | 策略分流 DNS         | **全 Resolve**    | ON                | ✅ GEOIP → DIRECT              | ✅ 非CN → Proxy           | 🟢 **较高**：默认机场 DoH，但前置 IP 规则可能产生额外真实解析 | 分流准确，但存在不必要解析         | 🟡 Windows 可用          |
| **③ ⭐ Balanced** | **策略分流 DNS**     | **混合 Resolve**  | **ON / OFF 可选** | ✅ 机场DoH → GEOIP CN → DIRECT | ✅ 机场DoH → 非CN → Proxy | 🟢 **高**：明确 CN 才走国内 DNS；未知/国外由机场 DoH 解析     | **隐私、分流准确性、复杂度最平衡** | 🟢 **游戏 Windows 推荐** |
...
优势： - DNS routing 最完整 - CN / 其他域名 DNS 分流 - 未知域名通过机场
DoH 做最终 GEOIP 判断 - DIRECT 可使用 `direct-nameserver`
获取更适合国内直连的解析 - 更适合关注游戏下载、国内 CDN 与流量方向的设备

缺点： - 配置复杂 - `respect-rules`、`direct-nameserver` 等增加排障变量

#### ⑥ Clash_2 Balanced

优势： - 结构更简单 - 默认机场 DoH，明确 CN 使用国内 DNS - 前置 IP 规则
`no-resolve` - 仅最后 `GEOIP,CN` 允许 Resolve - 未知国内仍可 DIRECT -
未知国外仍可 Proxy

如果③和⑥在游戏下载、CDN、延迟和兼容性上没有明显差异，优先考虑结构更简单的
**Clash_2 Balanced**。
...
这是本机 Mihomo 发起真实 DNS 查询，但查询交给机场海外
...
----------------------------------------------------------------------------------------------
  设备 / 目标             推荐方案                核心思想
----------------------- ----------------------- ----------------------------------------------
  **工作 Mac**            **④ Clash_2 Privacy**   不认识就代理，不为了节省代理流量解析未知域名

  **游戏 Windows**        **③ Clash_1 Pro**       完整 DNS 分流 + 国内 CDN / DIRECT 优化

  **游戏 Windows 备选**   **⑥ Clash_2 Balanced**  用更简单的架构实现未知 CN / 国外正确分流

Windows 最终建议通过 DNS、CDN、游戏下载和特殊域名访问测试，在 **Clash_1
Pro** 与 **Clash_2 Balanced** 之间选择。
...
``` yaml
https://<AIRPORT-DOH>/dns-query/<TOKEN>
```
