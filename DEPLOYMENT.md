...
若已有证书尚未到续期时间，安装器只会复用它；但会先确认 acme.sh 已安全保存 Cloudflare 续期凭据。凭据缺失时，当前安装输入的 Token 会触发一次受控强制签发以让 acme.sh 自行保存凭据；该操作失败或仍未保存时以 `acme_cf_token_not_persisted` 失败，不会把证书步骤记为完成。不要手工编辑或复制 acme.sh 的凭据文件，也不要反复强制签发。
