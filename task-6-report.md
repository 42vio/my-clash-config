...
- Hardened release-root validation so `private/releases` and `private/releases/<user>` must be real non-symlink directories before publish retention, history scans, release loads, or rollback. `_load_release()` now verifies the requested release resolves to the exact canonical managed path under that user root instead of trusting containment beneath a resolved symlink target.
