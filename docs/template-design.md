...
5. 候选先经过渲染校验（用真实渲染器生成一次 owner 配置）和秘密扫描；任一失败都不替换仓库中的当前模板。

同步报告只允许出现公开路径、是否更新、被忽略的顶层路径名；不得输出机场 URL、节点、Home 内容或凭据。
...
`private/**`、`.env` 和生成发布物均不受跟踪。3x-ui 的实际连接数据、机场订阅、订阅令牌、域名、节点地址和认证材料只可留在对应的私密文件、受保护的服务器运行时目录或交互式隐藏输入中。
